...
public:
    ...
    int f(vector<int> &arr, int ind, long long mul, vector<int> factors[], vector<unordered_map<long long, long long>> &dp)
    {
        if (ind == 0)
            return 1;
        if (dp[ind].find(mul) != dp[ind].end())
            return dp[ind][mul];
        int ways = f(arr, ind - 1, mul, factors, dp) % mod;
        if (arr[ind] != 0)
        {
            bool flag = true;
            for (auto &it : factors[ind])
                if (mul % it == 0)
                    flag = false;
            if (flag)
                ways = (ways + (arr[ind] * 1ll * f(arr, ind - 1, mul * 1ll * ind, factors, dp)) % mod) % mod;
        }
    ...
    int goodSubsets(vector<int> &arr, int n)
    {
        ...
        for (auto &i : arr)
        {
            if (((i % 4) == 0) || ((i % 9) == 0) || ((i % 16) == 0) || ((i % 25) == 0) || (i == 1 && ++ones))
                sec[i] = 0;
            else
                sec[i]++;
        }
